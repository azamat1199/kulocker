...
contract KuLocker is Ownable, ReentrancyGuard {
    ...
    uint256 public kcsFee = 5 ether;
    // base 1000, 1% = value * 10 / 1000
    uint256 public lpFeePercent = 10;

    // for statistic
    uint256 public totalKcsFees = 0;
    // withdrawable values
    uint256 public remainingKcsFees = 0;
    ...
    //Function to lock tokens in contract
    // _tokenAddress - Address of the locking token
    // _amount - Amount of the token
    // _unlockTime - Unlocking time in unix timestamp
    // _feeInKcs - If fee is taken in kcs or in token
    function lockTokens(
        ...
        require(!_feeInKcs || msg.value > kcsFee, 'KCS fee not provided');
        ...
        if (_feeInKcs) {
            totalKcsFees = totalKcsFees.add(msg.value);
            remainingKcsFees = remainingKcsFees.add(msg.value);
        } else {
    ...
    //Function to withdraw locked tokens
    //_id - id of the deposit
    ...
    //To get total balance of locked tokens of a particular token
    function getTotalTokenBalance(address _tokenAddress) view public returns (uint256)
    {
        return IERC20(_tokenAddress).balanceOf(address(this));
    }
    //To get balance of locked token by a particular wallet of a particular token
    function getTokenBalanceByAddress(address _tokenAddress, address _walletAddress) view public returns (uint256)
    {
        return walletTokenBalance[_tokenAddress][_walletAddress];
    }
    //Retreives all deposit ids
    function getAllDepositIds() view public returns (uint256[] memory)
    {
        return allDepositIds;
    }
    //To get details of a particular deposit id
    ...
    //For setting fees while locking. Default 5 kcs
    function setKcsFee(uint256 fee) external onlyOwner {
        require(fee > 0, 'Fee is too small');
        kcsFee = fee;
    }
    //For setting fee for lp deposit. Default 1 %
    function setLpFee(uint256 percent) external onlyOwner {
        require(percent > 0, 'Percent is too small');
        lpFeePercent = percent;
    }
    //For withdrawing accumulated fee from the contract
    function withdrawFees(address payable withdrawalAddress) external onlyOwner {
        if (remainingKcsFees > 0) {
            withdrawalAddress.transfer(remainingKcsFees);
            remainingKcsFees = 0;
        }
        uint len = tokenAddressesWithFees.length;
        for (uint i = 1; i <= len; i++) {
            address tokenAddress = tokenAddressesWithFees[len - i];
